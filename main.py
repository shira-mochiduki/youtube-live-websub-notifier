...
def get_due_upcoming_video_ids() -> List[str]:
    """
    upcoming のうち、scheduledStartTime の直前になった動画だけ返す。

    scheduledStartTime が取れなかった待機枠は、安全のため従来通り監視対象にする。
    """
    ...
    with db_connect() as con:
        rows = con.execute("""
            SELECT video_id, scheduled_start_time
            FROM videos
    ...
    if live_state == "upcoming":
        if scheduled_start_time:
            ...
            if scheduled:
                ...
                log.info(
                    "UPCOMING SCHEDULE id=%s scheduled=%s api-monitor-from=%s",
                    video_id,
                    scheduled.isoformat(),
                    wake_at.isoformat(),
                )
    ...
    while True:
        try:
            ...
            if due_ids or sleeping:
                log.info(
                    "UPCOMING-POLL due=%d sleeping=%d prestart=%dmin",
                    len(due_ids),
                    sleeping,
...
async def lifespan(app: FastAPI):
    ...
    log.info(
        ...
        "rss_safety=%ss live_probe=%ss websub_recovery=%ss public_base=%s",
        len(CHANNEL_MAP),
