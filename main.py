        ...
        if channels:
            ...
            for channel_id in channels:
                try:
                    ...
                    new_ids = [video_id for video_id in ids if video_id not in checked_ids]

                    log.info(
                        "RSS FALLBACK channel=%s entries=%d checked=%d new=%d",
                        channel_id,
                        len(ids),
                        len(checked_ids),
                        len(new_ids),
                    ...
                    if not new_ids:
                        log.info(
                            "RSS FALLBACK channel=%s no-new-video-ids -> API SKIP",
                            channel_id,
                        )
                        continue

                    # 新規Video IDを最大50件ずつまとめてvideos.listへ。
                    # RSSが9件なら9回ではなく1回のAPIリクエストになる。
                    ...
                    for video_id in new_ids:
                        ...
                        if not video:
                            log.warning(
                                "MISSING id=%s source=rss-fallback not-returned-by-api",
                                video_id,
                            )
                            # 同じ欠損IDを5分ごとに永久問い合わせしない。
                            mark_rss_checked(video_id, "missing")
                            continue

                        try:
                            result = await process_video_data(video, "rss-fallback")
                            mark_rss_checked(video_id, result or "processed")
                        except Exception:
                            # 一時的なDiscord/API後処理エラー時は未確認扱いにし、
                            # 次回RSSで再試行できるようcheckedには入れない。
                            log.exception(
