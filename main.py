...
# One long-lived client is shared by YouTube API, RSS, WebSub, Central Node,
...
async def process_video_data(video: dict, source: str):
    ...
    previous_live_event_emitted = bool(
        previous["live_event_emitted"]
    ...
    if live_state == "upcoming":
        ...
        # Historical ended streams should not generate live.ended merely
        # because Live Watcher encountered them for the first time.
        #
        # Allow live.ended only when this watcher had already observed the
        # stream as live, or when live.started was successfully emitted before.
        # The second condition also preserves retry behavior after a temporary
        # Central Node delivery failure.
        observed_live_before = (
            previous_state == "live"
            or previous_live_event_emitted
        )

        if not observed_live_before:
            log.info(
                "SKIP id=%s reason=historical-ended-no-live-observation previous_state=%s",
                video_id,
                previous_state or "-",
            )
            return "ended"
