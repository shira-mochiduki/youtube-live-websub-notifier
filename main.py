...
# YouTubeチャンネルの /live を使った現在LIVE確認。
# RSSやWebSubに配信中videoIdが出てこないケースの第三の安全網。
# Data APIではないため、このHTTP確認自体はYouTube Data API quotaを消費しない。
...
async def rss_fallback_monitor():
    ...
    RSS自体に現在LIVEのVideo IDが出ない/遅れるケースはlive_probe_monitorが補完する。
    """
    while True:
...
def extract_watch_video_id(url: str) -> Optional[str]:
    """
    最終URLが https://www.youtube.com/watch?v=VIDEO_ID の形ならVideo IDを返す。
    """
    try:
        parsed = urlparse(url)
...
async def probe_channel_live(channel_id: str) -> Optional[str]:
    """
    チャンネルの恒久 /live URL を確認する。
    配信中なら通常 watch?v=... へ遷移するため、そのVideo IDを取得する。

    これはYouTube Data APIではなく通常のHTTPアクセス。
    判定できなかった場合はNoneを返し、RSS/WebSub監視はそのまま継続する。
    """
...
async def live_probe_monitor():
    """
    WebSubやRSSが配信開始を拾わない場合の第三の安全網。

    60秒ごとに各チャンネルの /live を確認する。
    /live が watch?v=... を示した時だけData APIで公開状態・LIVE状態を最終確認する。

    すでに同じVideo IDでlive通知済みならData APIは呼ばない。
    """
    last_seen: Dict[str, Optional[str]] = {
        channel_id: None for channel_id in CHANNEL_MAP
    }
...
async def lifespan(app: FastAPI):
    ...
    log.info(
        "START channels=%d upcoming_poll=%ss rss_safety=%ss live_probe=%ss "
