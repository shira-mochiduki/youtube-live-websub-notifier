...
# このHTTP確認自体はYouTube Data API quotaを消費しない。
...
def classify_live(video: dict) -> str:
    """
    現在の配信状態は snippet.liveBroadcastContent を最優先で判定する。

    live     : 現在配信中
    upcoming : 待機枠 / 配信予定
    none     : 現在配信中でも待機中でもない

    liveStreamingDetails は終了済み判定の補助にのみ使う。
    """
...
def extract_watch_video_id(url: str) -> Optional[str]:
    """
    最終URLが watch?v=VIDEO_ID ならVideo IDを返す。
    """
    try:
        parsed = urlparse(url)
        ...
        video_id = (
            parse_qs(parsed.query).get("v") or [""]
        )[0].strip()
...
async def live_probe_monitor():
    ...
    60秒ごとに /live を確認。
    Video IDを取得できた場合だけData APIで最終確認する。
    """
            ...
            try:
                ...
                if saved and saved["live_notified"]:
                    if last_seen.get(channel_id) != video_id:
                        log.info(
                            "LIVE PROBE channel=%s id=%s "
                            "already-live-notified -> API SKIP",
                            channel_id,
                            video_id,
                        )
...
async def lifespan(app: FastAPI):
    ...
    log.info(
        ...
        "live_probe=%ss websub_recovery=%ss public_base=%s",
        len(CHANNEL_MAP),
